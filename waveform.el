...
(defun waveform--update-position ()
  (if (mpv-live-p)
      (if (get-buffer-window waveform--current-mpv-buffer)
          (condition-case _
...
(defvar waveform--position-timer nil)
(defvar waveform--current-mpv-buffer nil "Which buffer has the active MPV process.")
